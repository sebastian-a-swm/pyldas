        ...
        for bl in np.arange(n_blocks, dtype='int') * n_fields:
            if n_fields == 32 and n_blocks == 28:
                ...
                res.append({'descr': s(lines[bl + 0]),
                            ...
                            'flistname': s(lines[bl + 24]),
                            'errstd': float(lines[bl + 25]),
                            'errstd_file': s(lines[bl + 26]),
                            'path_errstd': s(lines[bl + 27]),
                            'std_normal_max': float(lines[bl + 28]),
                            'zeromean': b(lines[bl + 29]),
                            'coarsen_pert': b(lines[bl + 30]),
                            'xcorr': float(lines[bl + 31]),
                            'ycorr': float(lines[bl + 32]),
                            'adapt': int(lines[bl + 33])})
            else:
                res.append({'descr': s(lines[bl + 0]),
                            'species': int(lines[bl + 1]),
                            'orbit': int(lines[bl + 2]),
